...
# Oririce Dimensions
step_thickness = 50 # vertical
step_length = 50 # horizontal
...
def pressure_poisson(p,b):
    ...
    for _ in range(nit): # we iterate for pressure to get more accurate steadx state result
        ...
        p[step_thickness, 0:step_length] = p[step_thickness+1, 0:step_length]
        p[0:step_thickness, step_length] = p[0:step_thickness, step_length+1]
...
def orifice_flow(u, v, p):
    ...
    for _ in range(nt):
        ...
        # Orifice in the middle
        u[0:step_thickness, 0:step_length] = 0
        v[0:step_thickness, 0:step_length] = 0
...
plt.contour(X, Y, p, cmap=cm.seismic)
plt.contourf(X, Y, p, alpha=0.5, cmap=cm.seismic)
plt.colorbar()
plt.streamplot(X,Y,u,v)
# plt.quiver(X,Y,u,v)
# plotting velocity field
plt.title('Back Step Flow')
plt.xlabel('X')
plt.ylabel('Y')
plt.show()